...
const uiDescriptions = {
  ...
  "decision-prototype": "用最小可丢弃逻辑或 UI 原型验证一个设计决策",
  ...
  "architecture-deepening-analysis": "分析接口、调用方与测试摩擦并比较可迁移可回滚的深化方案",
...
const uiPrompts = {
  ...
  "decision-prototype": "使用 $decision-prototype 以最小非生产原型验证一个逻辑或 UI 决策，不把原型当作正式实现。",
  ...
  "architecture-deepening-analysis": "使用 $architecture-deepening-analysis 只读分析架构摩擦并比较迁移方案，不执行重构。",
