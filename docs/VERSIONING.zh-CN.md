...
当前项目发布版本是 **0.4.1 stable**。未变化的 Skill 集合保留 **317/317 次真实 Codex 调用 PASS** 证据，包括 200 项核心路由、44 个双语工作流步骤、61 项模式与原型三态判定，以及 12 项权限边界。机器可读的 317 次调用报告会如实标识实际执行版本 v0.4.0；v0.4.1 不把它冒充为新一轮运行。
...
v0.4.1 只改变仓库文档和远程 Release 验证器，不改变 Pack、插件或 Skill 的公开契约。因此 Pack/插件配对继续保留 v0.4.0 的组件基线，并保持严格一致。
...
0.4.1 项目正式稳定版中，4 个 active Pack 和 40 个 active Skill 仍全部为 `beta`。模板 Pack 及其中的模板 Skill 继续保持 `draft`；项目通道 stable 也不会自动提升组件成熟度。

仓库校验、activation contract、确定性构建和真实模型测试分别提供不同证据。未变化的 Skill 集合在 v0.4.0 达到 317/317 Live-model PASS，没有失败或阻塞调用；机器可读报告保留全部案例和有界证据。v0.4.1 重新运行确定性与远程 Release 门禁，并在发布后执行路由和权限 smoke tests。
...
Repo Doctor Skills 仍处于 1.0 之前。0.4.0 是 v0.3.0 之后的 minor 版本：新增两个向后兼容 Skill，扩展工作流和平台安装产物，并增强六个既有 Skill，且未删除或重命名 canonical slug。0.4.1 是 patch：只修改文档、可复现安装指引和 Release 验证，不改变 Skill 契约。1.0.0 必须是明确的产品决策。
