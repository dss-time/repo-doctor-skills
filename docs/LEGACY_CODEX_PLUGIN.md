...
The generated Repo Doctor distribution contains 27 scoped workflow and specialist engineering Skills:
...
The current Codex CLI supports `--ref` for Git marketplace sources. In a graphical host, use source `dss-time/repo-doctor-skills` or its full Git URL, set the Git ref to `v0.6.1` for stable use, leave the sparse path empty, and then install the `Repo Doctor` plugin.
