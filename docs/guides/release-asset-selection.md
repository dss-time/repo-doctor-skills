...
v0.4.1 publishes 40 assets:

- 37 versioned single-Skill ChatGPT ZIPs: 27 `rd-*`, 8 `pt-*`, and 2 `sm-*`;
- `Repo-Doctor-Skills-v0.4.1-RELEASE_NOTES.md`;
- `Repo-Doctor-Skills-v0.4.1-RELEASE_MANIFEST.txt`;
- `SHA256SUMS-v0.4.1.txt`.
...
|---|---|---|
| Route an engineering request | `rd-repo-doctor-router-v0.4.1.zip` | `repo-doctor-router` |
| Diagnose a qualified non-CI runtime bug | `rd-bug-root-cause-analysis-v0.4.1.zip` | `bug-root-cause-analysis` |
| Review a code change | `rd-safe-code-review-v0.4.1.zip` | `safe-code-review` |
| Analyze architecture friction | `rd-architecture-deepening-analysis-v0.4.1.zip` | `architecture-deepening-analysis` |
| Draft an evidence-aware report | `pt-report-writer-v0.4.1.zip` | `report-writer` |
| Review a Word or general document | `pt-document-review-v0.4.1.zip` | `document-review` |
| Review a PDF | `pt-pdf-review-v0.4.1.zip` | `pdf-review` |
| Clean Excel or other spreadsheet data | `pt-spreadsheet-data-cleaning-v0.4.1.zip` | `spreadsheet-data-cleaning` |
| Create a presentation outline | `pt-presentation-outline-v0.4.1.zip` | `presentation-outline` |
| Author a repository-standard Skill | `sm-skill-authoring-v0.4.1.zip` | `skill-authoring` |
| Audit Skill release quality | `sm-skill-quality-audit-v0.4.1.zip` | `skill-quality-audit` |

Other assets follow the same published naming rule:

- `rd-<repo-doctor-slug>-v0.4.1.zip`;
- `pt-<productivity-slug>-v0.4.1.zip`;
- `sm-<skill-maintainer-slug>-v0.4.1.zip`.
...
gh release download v0.4.1 \
  ...
  -p 'rd-repo-doctor-router-v0.4.1.zip' \
  -p 'Repo-Doctor-Skills-v0.4.1-RELEASE_MANIFEST.txt' \
  -p 'SHA256SUMS-v0.4.1.txt'
```

`SHA256SUMS-v0.4.1.txt` covers all 37 Skill ZIPs. If only one ZIP is present locally, verify that file against its matching line:

```bash
expected=$(awk '$2 == "rd-repo-doctor-router-v0.4.1.zip" { print $1 }' SHA256SUMS-v0.4.1.txt)
actual=$(shasum -a 256 rd-repo-doctor-router-v0.4.1.zip | awk '{ print $1 }')
...
```bash
npm run release:verify-remote -- --tag v0.4.1
```
...
- Pin stable use to `v0.4.1`; use `main` only for deliberate development testing.
